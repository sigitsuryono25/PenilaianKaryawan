					...
					<?php foreach ($pengumuman as $key => $p) { ?>
						<tr>
							...
							<td>
								...
								if (!empty($file)) { ?>
									<ol list-type="1" class="px-3">
										<?php foreach ($file as $k => $f) { ?>
											<li>
												<a target="_blank" href="<?= base_url('assets/public/pengumuman/' . $f) ?>">Lampiran-<?= $k + 1 ?></a>
											</li>
										<?php } ?>
									</ol>
								<?php } ?>
							...
							<td>
								...
								<a href="<?= site_url('pengumuman/action-delete?id=' . $p->id) ?>" onclick="return confirm('Hapus pengumuman ini?')" class="btn btn-danger btn-sm btn-block">
